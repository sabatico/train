...
- **Model / role policy:** routine work → cheap model (Haiku-class); mid → mid model (Sonnet-class); hard/risky/pedagogy-critical → strong model (the lead). **Tests are authored by a DIFFERENT model/agent than the builder** (the builder writes ZERO tests for its own code — see `docs/sops/test-and-coverage.md`). A **scribe** documents each finished item.
